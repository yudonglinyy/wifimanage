...
class Echo(protocol.Protocol): ### Set custom protocol class name
	...
	def connect(self):
		...
		log.msg('%s %s CONNECT %s %s %s %s %s' % (self.session, self.remote_host.type, self.local_host.host, self.local_host.port, self.factory.name, self.remote_host.host, self.remote_host.port))
                log_file_display=open('/root/log/honey.log','a')
                log_file_display.write('%s %s %s CONNECT %s %s' % (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())), self.remote_host.type, self.local_host.port, self.factory.name, self.remote_host.host))
                log_file_display.write('\n')
                log_file_display.close()
